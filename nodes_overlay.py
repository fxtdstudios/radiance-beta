...
class RadianceMetadataOverlay:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "position": (["Bottom Bar (Slate)", "Top Left", "Top Right", "Bottom Left", "Bottom Right"],),
                "font_size": ("INT", {"default": 20, "min": 10, "max": 100}),
                "opacity": ("FLOAT", {"default": 0.7, "min": 0.0, "max": 1.0, "step": 0.1}),
            },
            "optional": {
                "user_text": ("STRING", {"default": "", "multiline": True, "placeholder": "Additional notes..."}),
                "report_text": ("STRING", {"forceInput": True, "multiline": True, "placeholder": "Connect QC Report here..."}),
            }
        }
    ...
    def overlay_metadata(self, image: torch.Tensor, enabled: bool, project: str, shot: str, 
                         frame: int, timecode: str, position: str, font_size: int, 
                         opacity: float, user_text: str = "", report_text: str = ""):
        ...
        for i in range(batch_size):
            ...
            metadata_lines = []
            if project: metadata_lines.append(f"PRJ: {project}")
            if shot: metadata_lines.append(f"SHT: {shot}")
            if frame is not None: 
                current_frame = frame + i # Increment frame number for batch
                metadata_lines.append(f"FRM: {current_frame:04d}")
            if timecode: metadata_lines.append(f"TC : {timecode}") # TODO: Increment TC?
            ...
            if position == "Bottom Bar (Slate)":
                ...
                # Draw Extra Text
                if full_extra_text:
                    draw.text((margin, text_y + font_size + 10), full_extra_text, font=font, fill=sub_text_color)
                ...
                # Estimate box size
                bbox = draw.multiline_textbbox((0, 0), text_to_draw, font=font, align="left")
                ...
                # Draw Text
                draw.multiline_text((box_x, box_y), text_to_draw, font=font, fill=text_color, align="left")
            ...
            if c == 1:
                ...
                overlay_rgb = (overlay_rgb[..., 0] * 0.299 + overlay_rgb[..., 1] * 0.587 + overlay_rgb[..., 2] * 0.114).unsqueeze(-1)
            
            result_frame = image[i] * (1.0 - overlay_alpha) + overlay_rgb * overlay_alpha
