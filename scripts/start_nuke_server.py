...
_BLOCKED_PATTERNS = [
    ...
    'os.rmdir',  'os.makedirs',  'os.rename',
...
def handle_client(conn):
    ...
    try:
        ...
        if len(header_data) == 9 and header_data[:4] == RADIANCE_MAGIC:
            ...
            command = (header_data + remaining).decode('utf-8', errors='replace').strip()
        ...
        if command:
                    ...
                    try:
                        ...
                        if lines:
                            ...
                            if last.startswith("'") or last.startswith('"'):
                                try:
                                    result_box[0] = str(eval(last, safe_globals))
                                except Exception:
                                    result_box[0] = "OK"
                            else:
                                result_box[0] = "OK"
            ...
            response = error_box[0] if error_box[0] else (result_box[0] if result_box[0] else "ERROR: Timeout")
...
def fetch_last_history_item():
    ...
    try:
        with urllib.request.urlopen(f"{COMFY_URL}/history") as response:
...
def find_cinematic_node(graph):
    ...
    if nid: return nid, ndata, 'ENCODER'
    
    nid, ndata = find_node_by_class(graph, 'RadianceCinemaStudio')
    if nid: return nid, ndata, 'STUDIO'
    ...
    for node in graph.values():
        if 'inputs' in node:
            if 'seed' in node['inputs']:
                 node['inputs']['seed'] = random.randint(1, 18446744073709551615)
            elif 'noise_seed' in node['inputs']:
                 node['inputs']['noise_seed'] = random.randint(1, 18446744073709551615)
...
class RadiancePromptPanel(nukescripts.PythonPanel):
    def __init__(self):
        ...
        # --- Data Lists ---
        self.cameras = ["None", "ARRI Alexa 35", "ARRI Alexa Mini LF", "ARRI Alexa 65 (IMAX)", "Sony Venice 2", "RED V-Raptor XL", "Panavision Millennium DXL2", "IMAX 15/70mm Film Camera"]
        self.lenses = ["None", "50mm Standard Prime", "35mm Classic Wide", "85mm Portrait Prime", "24mm Wide Angle", "Anamorphic Lens", "ARRI/Zeiss Signature Prime", "Panavision Primo 70", "Cooke S7/i Full Frame"]
        self.apertures = ["None", "f/1.2 (Dreamy Bokeh)", "f/1.8 (Soft Background)", "f/2.8 (Cinematic Separation)", "f/4.0 (Balanced)", "f/5.6 (Sharp Subject)", "f/8.0 (Deep Focus)"]
        self.framings = ["None", "Medium Shot (MS)", "Close-Up (CU)", "Wide Shot (WS)", "Extreme Wide Shot (EWS)", "Low Angle (Hero Shot)"]
        
        self.presets = [
            "SDXL Square (1:1)", "SDXL Portrait (3:4)", "SDXL Landscape (4:3)", "SDXL Wide (16:9)", 
            "FLUX 1MP Square", "FLUX 1MP Wide (16:9)", "FLUX 1MP Portrait (9:16)", 
            "FLUX 2K Square (1:1)", "FLUX 2K Wide (16:9)", "FLUX 2K DCI (1.90:1)",
            "1080p Full HD (16:9)", "4K UHD (16:9)", "4K DCI (1.90:1)", "2K DCI Scope (2.39:1)",
            "Instagram Portrait (4:5)", "Instagram Story/Reel (9:16)"
        ]
        
        self.samplers = ["euler", "euler_ancestral", "heun", "dpm_2", "dpm_2_ancestral", 
                         "dpmpp_2s_ancestral", "dpmpp_2m", "dpmpp_2m_sde", "dpmpp_sde", "uni_pc"]
        self.schedulers = ["normal", "karras", "exponential", "sgm_uniform", "simple", "ddim_uniform"]
        
        # RadianceDepthMapGenerator inputs
        self.depth_models = ["Small (25M - Fast)", "Base (98M - Balanced)", "Large (335M - Best)"]
    ...
    def sync_from_last(self):
        ...
        if nid:
            ...
            camera_val = inputs.get('camera_type') if ntype == 'ENCODER' else inputs.get('camera')
            lens_val = inputs.get('lens_focal') if ntype == 'ENCODER' else inputs.get('focal_length')
            ap_val = inputs.get('aperture_dof') if ntype == 'ENCODER' else inputs.get('aperture')
            frame_val = inputs.get('framing') if ntype == 'ENCODER' else "None"

            if camera_val in self.cameras: self.camera_knob.setValue(camera_val)
            if lens_val in self.lenses: self.lens_knob.setValue(lens_val)
            if ap_val in self.apertures: self.aperture_knob.setValue(ap_val)
            if frame_val in self.framings: self.framing_knob.setValue(frame_val)
        ...
        if sid:
            ...
            if sname in self.samplers: self.sampler_knob.setValue(sname)
            
            schname = sinputs.get('scheduler')
            if schname in self.schedulers: self.scheduler_knob.setValue(schname)
        ...
        if did:
            ...
            self.depth_model_knob.setValue(dinputs.get('model_size', 'Base (98M - Balanced)'))
