...
class RadianceGrade:
        ...
        return {
            "required": {
                "image": (
                    "IMAGE",
                    {
                        "tooltip": "Input image to grade. Processed in 32-bit float precision for maximum quality."
                    },
                ),
                "preset": (
                    preset_names,
            ...
            "optional": {
                ...
                "gamma_r": (
                    "FLOAT",
                    {
                        ...
                        "max": 5.0,
                        "step": 0.001,
                        "tooltip": "Red channel gamma (midtone power). >1 = brighter midtones, <1 = darker midtones (Nuke style).",
                    },
                ),
                "gamma_g": (
                    "FLOAT",
                    {
                        ...
                        "max": 5.0,
                        "step": 0.001,
                        "tooltip": "Green channel gamma (midtone power). >1 = brighter midtones, <1 = darker midtones (Nuke style).",
                    },
                ),
                "gamma_b": (
                    "FLOAT",
                    {
                        ...
                        "max": 5.0,
                        "step": 0.001,
                        "tooltip": "Blue channel gamma (midtone power). >1 = brighter midtones, <1 = darker midtones (Nuke style).",
                    },
                ),
                # Gain (Highlights)
                "gain_r": (
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "Information about applied grade settings.",
    )
    FUNCTION = "grade"
    ...
    def grade(
        ...
        if (
            preset != "None (Custom)"
            and preset in GRADE_PRESETS
            and preset_strength > 0
        ):
        ...
        if gamma_r != 1.0 or gamma_g != 1.0 or gamma_b != 1.0:
            epsilon = 1e-8
            # v1.1.1 FIX: Use sign-preserving gamma to handle negative values (lifted blacks) without clamping

            if gamma_r != 1.0:
                img[..., 0] = torch.sign(img[..., 0]) * torch.pow(
                    torch.abs(img[..., 0]) + epsilon, 1.0 / gamma_r
                )
            if gamma_g != 1.0:
                img[..., 1] = torch.sign(img[..., 1]) * torch.pow(
                    torch.abs(img[..., 1]) + epsilon, 1.0 / gamma_g
                )
            if gamma_b != 1.0:
                img[..., 2] = torch.sign(img[..., 2]) * torch.pow(
                    torch.abs(img[..., 2]) + epsilon, 1.0 / gamma_b
                )
...
NODE_CLASS_MAPPINGS = {"RadianceGrade": RadianceGrade}

NODE_DISPLAY_NAME_MAPPINGS = {"RadianceGrade": "◎ Radiance Grade"}
