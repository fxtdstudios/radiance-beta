...
class RadianceGrade:
    ...
    def grade(self, image: torch.Tensor, preset: str = "None (Custom)",
        ...
        if gamma_r != 1.0 or gamma_g != 1.0 or gamma_b != 1.0:
            ...
            if gamma_r != 1.0:
                img[..., 0] = torch.sign(img[..., 0]) * torch.pow(torch.abs(img[..., 0]) + epsilon, 1.0 / gamma_r)
            if gamma_g != 1.0:
                img[..., 1] = torch.sign(img[..., 1]) * torch.pow(torch.abs(img[..., 1]) + epsilon, 1.0 / gamma_g)
            if gamma_b != 1.0:
                img[..., 2] = torch.sign(img[..., 2]) * torch.pow(torch.abs(img[..., 2]) + epsilon, 1.0 / gamma_b)
