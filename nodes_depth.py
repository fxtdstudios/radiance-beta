...
def download_and_load_model(model_size: str, device: torch.device):
    ...
    with _cache_lock:
        ...
        try:
            ...
            # Download and load
            processor = AutoImageProcessor.from_pretrained(model_id, revision="main")  # nosec B615
            model = AutoModelForDepthEstimation.from_pretrained(model_id, revision="main")  # nosec B615
            model.to(device)
            model.eval()
