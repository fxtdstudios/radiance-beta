...
- **`docs/developer.md` rewritten.** Its "Adding A Node" steps were wrong: they
  told you to hand-edit the node reference and the coverage ledger, which are
  generated by `tools/generate_docs.py` and guarded by a test that fails if you
  edit them. The new checklist covers the ratchets a change actually has to
  satisfy — the docs generator, `node_keys_snapshot.json`, the README counts,
  cross-module duplicate registration, and the menu-section classifier.
