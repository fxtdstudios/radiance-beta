"""
═══════════════════════════════════════════════════════════════════════════════
                    RADIANCE — UNIVERSAL VFX IO v2.0
═══════════════════════════════════════════════════════════════════════════════
...
def _extract_audio_ffmpeg(video_path: str) -> dict | None:
    ...
    try:
        ...
        waveform = samples.reshape(samples_per_channel, channels).T  # (channels, samples)
        waveform_tensor = torch.from_numpy(waveform.copy()).unsqueeze(0)  # (1, channels, samples)
        
        logger.info(f"Audio extracted: {sample_rate}Hz, {channels}ch, {samples_per_channel} samples")
...
class RadianceReadVideo:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "frame_count": ("INT", {"default": 0, "min": 0, "max": 999999, "tooltip": "0 = Load all frames"}),
                "input_colorspace": (INPUT_COLORSPACES, {"default": "sRGB (Standard)"}),
                "frame_stride": ("INT", {"default": 1, "min": 1, "max": 100, "tooltip": "Skip every Nth frame"}),
    ...
    DESCRIPTION = "Professional video reader with color transforms and audio extraction."

    def read_video(self, video_path, start_frame, frame_count, input_colorspace, frame_stride, extract_audio=True):
        ...
        if start_frame >= total_frames:
            cap.release()
            raise ValueError(f"Start frame {start_frame} is beyond end of video ({total_frames} frames)")
        ...
        logger.info(f"Reading video: {os.path.basename(video_path)} | {width}x{height} @ {fps:.2f}fps | Start: {start_frame} | Count: {frames_to_read} | IDT: {input_colorspace}")
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "pattern": ("STRING", {"default": "*.exr", "tooltip": "Glob pattern (e.g. *.png, Shot_*.exr)"}),
                "start_frame": ("INT", {"default": 1001, "min": 0, "max": 999999, "tooltip": "Frame number in filename, or 0 for auto-detect first"}),
                "frame_limit": ("INT", {"default": 0, "min": 0, "max": 99999, "tooltip": "0 = Load all"}),
                "input_colorspace": (INPUT_COLORSPACES, {"default": "Linear (sRGB)"}),
                "fps": ("FLOAT", {"default": 24.0, "min": 1.0, "max": 120.0, "step": 0.001, "tooltip": "Assumed FPS for image sequences"}),
            },
        }

    RETURN_TYPES = ("IMAGE", "MASK", "INT", "INT", "INT", "FLOAT", "STRING")
    RETURN_NAMES = ("IMAGE", "MASK", "frame_count", "width", "height", "fps", "filename_list")
    FUNCTION = "read_sequence"
    CATEGORY = "FXTD Studios/Radiance/IO"
    DESCRIPTION = "Professional image sequence reader with EXR/HDR/PNG support and color transforms."

    def read_sequence(self, folder_path, pattern, start_frame, frame_limit, input_colorspace, fps=24.0):
        ...
        if start_frame > 0:
            ...
            for idx, f in enumerate(files):
                if str_frame in os.path.basename(f):
                    ...
                    break
            if not found:
                logger.warning(f"Could not find file with frame number {start_frame}, starting from first file.")
        ...
        return (batch_img, batch_mask, len(images), img_width, img_height, fps, filenames_str)
...
class RadianceWrite:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "quality": ("INT", {"default": 10, "min": 0, "max": 51, "tooltip": "CRF for H.264/5 (lower=better). JPEG quality (0-100) for JPEG sequences. Unused for ProRes/PNG/EXR."}),
    ...
    def write(self, images, filename_prefix, output_format, fps, quality, output_color_space, subfolder="", audio=None, prompt=None, extra_pnginfo=None):
        ...
        logger.info(f"Writing: {filename_prefix} | {output_format} | {output_color_space} | {len(images_np)} frames")
        
        # ─── Route to appropriate writer ───────────────────────────────
        if output_format.startswith("Video"):
            filepath = self._write_video(images_np, filename_prefix, output_format, fps, quality, output_color_space, full_output_dir, timestamp, audio)
        elif output_format.startswith("Image Sequence"):
            filepath = self._write_sequence(images_np, filename_prefix, output_format, quality, full_output_dir, timestamp)
        elif "GIF" in output_format:
            filepath = self._write_gif(images_np, filename_prefix, fps, full_output_dir, timestamp)
        elif "WEBP" in output_format:
            filepath = self._write_webp(images_np, filename_prefix, fps, full_output_dir, timestamp)
        else:
    ...
    def _write_video(self, images_np, prefix, fmt, fps, quality, color_space, output_dir, ts, audio):
        ...
        if "H.264" in fmt:
            ...
            ffmpeg_params = ["-x265-params", f"profile=main10:crf={quality}", "-tag:v", "hvc1"]
        ...
        if color_space in color_map:
            ...
            ffmpeg_params += ["-color_primaries", prim, "-color_trc", trc, "-colorspace", space]

        filepath = os.path.join(output_dir, f"{prefix}_{ts}.{ext}")
        
        iio.imwrite(
            filepath, video_data,
            fps=fps, codec=codec_name,
    ...
    def _mux_audio(self, video_path, audio, fps):
        ...
        try:
            ...
            # Mux with FFmpeg
            with tempfile.NamedTemporaryFile(suffix=os.path.splitext(video_path)[1], delete=False) as tmp:
                tmp_out = tmp.name
...
# Keep legacy node for backward compat
class RadianceWriteVideo(RadianceWrite):
    """Legacy alias — use RadianceWrite instead."""
    CATEGORY = "FXTD Studios/Radiance/IO"
    DESCRIPTION = "Legacy video writer. Use 'Radiance Write' for full format support."
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE",),
                "filename_prefix": ("STRING", {"default": "Radiance_Export"}),
                "fps": ("FLOAT", {"default": 24.0, "min": 1.0, "max": 120.0}),
                "codec": (VIDEO_CODECS, {"default": "ProRes 422 HQ (10-bit)"}),
                "quality": ("INT", {"default": 10, "min": 0, "max": 51}),
                "output_color_space": (INPUT_COLORSPACES, {"default": "sRGB (Standard)"}),
                "subfolder": ("STRING", {"default": ""}),
            },
            "optional": {
    ...
    # Map old codec names to new output_format names
    _CODEC_MAP = {
        "H.264 High (8-bit)":       "Video — MP4 (H.264)",
        "H.265 Main10 (10-bit)":    "Video — MP4 (H.265 10-bit)",
        "ProRes 422 HQ (10-bit)":   "Video — MOV (ProRes 422 HQ)",
        "ProRes 4444 (12-bit)":     "Video — MOV (ProRes 4444)",
        "ProRes 4444 XQ (12-bit)":  "Video — MOV (ProRes 4444 XQ)",
    }
    
    def write_video(self, images, filename_prefix, fps, codec, quality, output_color_space, subfolder="", audio=None, prompt=None, extra_pnginfo=None):
        output_format = self._CODEC_MAP.get(codec, "Video — MP4 (H.264)")
        return self.write(images, filename_prefix, output_format, fps, quality, output_color_space, subfolder, audio, prompt, extra_pnginfo)
...
NODE_CLASS_MAPPINGS = {
    ...
    "RadianceWriteVideo": RadianceWriteVideo,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "RadianceWriteVideo": "◎ Radiance Write (Video) [Legacy]",
}
