...
class RadianceQC:
    ...
    def analyze(self, image: torch.Tensor, 
                ...
                fail_on_errors: bool = False) -> Tuple:
        ...
        try:
            ...
            if image.dim() != 4:
                return self._create_error(f"Invalid shape: expected (B,H,W,C), got {image.shape}")
            ...
            text_report_lines = ["╔═══════════════════════════════════════════╗"]
            text_report_lines.append("║     RADIANCE QC REPORT v2.0              ║")
            ...
            for i in range(B):
                ...
                if crushed_pct > 0:
                    frame_report.append(f"  ⚠ CRUSHED: {crushed_pct:.2f}% pixels < {black_threshold}")
                    frame_pass = False
                ...
                if clipped_pct > 0:
                    frame_report.append(f"  ⚠ CLIPPED: {clipped_pct:.2f}% pixels > {white_threshold}")
                    frame_pass = False
                ...
                if banding_pct >= banding_threshold:
                    frame_report.append(f"  ⚠ BANDING: {banding_pct:.2f}% risk (threshold: {banding_threshold}%)")
                ...
                if noise is not None:
                    ...
                    if noise_score < 5.0:
                        frame_report.append(f"  ℹ NOISE: {noise_score:.2f}/100 (unusually clean, possible over-denoising)")
                    elif noise_score > 30.0:
                        frame_report.append(f"  ℹ NOISE: {noise_score:.2f}/100 (high noise level)")
                ...
                if artifacts is not None:
                    ...
                    if artifact_score >= 10.0:
                        frame_report.append(f"  ⚠ ARTIFACTS: {artifact_score:.2f}/100 compression artifacts detected")
                ...
                if focus is not None:
                    ...
                    if focus_score < 20.0:
                        frame_report.append(f"  ⚠ FOCUS: {focus_score:.2f}/100 (low sharpness)")
                ...
                if overlay_opacity > 0:
                    ...
                    blended = target * (1.0 - overlay_opacity) + overlay * overlay_opacity
...
class RadianceQCExport:
    ...
    def export_report(self, qc_report_json: str, output_path: str, 
                     filename_prefix: str, export_format: str) -> Tuple[str]:
        try:
            import os
            ...
            if export_format in ["csv", "all"]:
                ...
                with open(csv_path, 'w') as f:
                    ...
                    for frame in report.get("frames", []):
                        ...
                        f.write(f"{frame_num},{status},{crushed:.2f},{clipped:.2f},{gamut:.2f},{banding:.2f}\n")
...
"""
        ...
        for frame in report.get("frames", []):
            ...
            html += f"<td>{checks.get('crushed_blacks', {}).get('percentage', 0):.2f}%</td>"
            html += f"<td>{checks.get('clipped_whites', {}).get('percentage', 0):.2f}%</td>"
            html += f"<td>{checks.get('gamut', {}).get('out_of_gamut_pct', 0):.2f}%</td>"
