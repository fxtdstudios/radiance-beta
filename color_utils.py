...
REDWIDEGAMUT_TO_ACESCG = np.array([
    [0.5510, 0.2114, 0.2376],
    [0.0089, 0.9996, -0.0086],
    [-0.0886, -0.0583, 1.1470]
], dtype=np.float32)
...
LOGC3_EI_PARAMS = {
    ...
    800:  (0.010591, 5.555556, 0.052272, 0.247190, 0.385537, 5.367655, 0.092809),  # Standard
...
def aces2_tonemap(img: np.ndarray, peak_luminance: float = 1000.0, 
                   mid_gray: float = 0.18, contrast: float = 1.0) -> np.ndarray:
    """
...
def aces2_gamut_compress(img: np.ndarray, threshold: float = 0.75, 
                          power: float = 1.2) -> np.ndarray:
    """
    ...
    if np.any(compress_mask):
        ...
        compressed_distance = threshold + (1.0 - threshold) * (1.0 - np.power(1.0 - excess, power))
