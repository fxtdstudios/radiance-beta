...
def _build_prose_prompt(
    ...
    if weight_mode == "technique_first" and c(camera):
        cam_clean = _clean_for_prose(camera)
        parts.append(f"Photographed on {cam_clean}, {subject}.")
    elif weight_mode == "subject_first":
        parts.append(f"{subject}, the central focus of this scene.")
    elif c(framing):
        parts.append(f"{_clean_for_prose(framing)} of {subject}.")
    else:
    ...
    if c(camera) and weight_mode != "technique_first":
        tech_parts.append(f"captured on {_clean_for_prose(camera)}")
    if c(lens):
        tech_parts.append(f"through a {_clean_for_prose(lens)}")
    if c(aperture):
        tech_parts.append(f"at {_clean_for_prose(aperture)}")
    if c(shutter):
        tech_parts.append(f"with {_clean_for_prose(shutter)} shutter")
    if tech_parts:
        if len(tech_parts) == 1:
            parts.append(f"The image was {tech_parts[0]}.")
        else:
            # First element starts the sentence; rest join naturally
            parts.append(f"The image was {', '.join(tech_parts[:-1])} and {tech_parts[-1]}.")

    # 5. Lighting
    if c(lighting):
        parts.append(f"The scene is bathed in {_clean_for_prose(lighting)}.")

    # 6. Color and finishing
    finish = []
    if c(color_grading): finish.append(f"color graded in {_clean_for_prose(color_grading)}")
    if c(film_stock):    finish.append(f"with the texture of {film_stock}")
    ...
    # 8. Aspect ratio
    if c(aspect_ratio):
        parts.append(f"Composed for {_clean_for_prose(aspect_ratio)} format.")
