...
try:
    from ..color_utils import (
        linear_to_logc3, linear_to_logc4, linear_to_slog3,
        linear_to_vlog, linear_to_canonlog3, linear_to_acescct,
        srgb_to_linear, linear_to_srgb,
        apply_matrix_transform,
        AWG3_TO_ACESCG, AWG4_TO_ACESCG, SGAMUT3_CINE_TO_ACESCG,
        ACESCG_TO_SRGB, ACESCG_TO_P3D65
    )
...
class LoadImageEXR:
    ...
    def load(self, file_path: str, input_folder_file: str = "", exposure_adjust: float = 0.0) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, str]:
        ...
        try:
            ...
            if 'R' in channels and 'G' in channels and 'B' in channels:
                ...
                if 'A' in channels:
                    ...
                    alpha_img = np.frombuffer(a_str, dtype=np.float32).copy().reshape(height, width)
                else:
                ...
                c = np.frombuffer(ch_str, dtype=np.float32).copy().reshape(height, width)
            ...
            if depth_ch:
                ...
                depth_img = np.frombuffer(z_str, dtype=np.float32).copy().reshape(height, width)
            else:
...
class LoadImageEXRSequence:
    ...
    def load_sequence(self, folder_path: str, file_pattern: str = "*.exr",
                      ...
                      exposure_adjust: float = 0.0) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, str, int]:
        ...
        if max_frames > 0 and num_files > max_frames:
            ...
            estimated_gb = (num_files * 8 * 1024 * 1024 * 4) / (1024**3)  # Rough estimate for 4K float32
            logger.warning(
        ...
        try:
            ...
            if first_alpha is None:
                 first_alpha = np.ones((first_img.shape[0], first_img.shape[1]), dtype=np.float32)
                 
            if first_depth is None:
                 first_depth = np.zeros((first_img.shape[0], first_img.shape[1]), dtype=np.float32)
            ...
            raise RuntimeError(f"Could not determine sequence format from first frame: {e}")
        ...
        for i, file_path in enumerate(files):
            try:
                ...
                if i == 0:
                    ...
                    if img is None:
                        logger.warning(f"Failed to load {file_path}, frame will be black.")
                        continue
                    ...
                    if len(img.shape) == 2:
                        ...
                        if img.shape[2] == 4:
                            if alpha_img is None: alpha_img = img[:, :, 3]
                            img = img[:, :, :3]
                        elif img.shape[2] == 1:
                            img = np.repeat(img, 3, axis=2)
                ...
                # Check dimensions
                if img.shape[:2] != (h, w):
                    logger.warning(f"Skipping frame {i}: Dimension mismatch {img.shape[:2]} vs {(h, w)}")
                    continue
...
class SaveImage16bit:
    ...
    def INPUT_TYPES(cls) -> Dict[str, Any]:
        return {
            "required": {
                ...
                "tonemap": (["Clip (Standard)", "Compress (LogC4)", "Compress (Reinhard)"], {"default": "Clip (Standard)"}),
            }
        }
    ...
    def save(self, images: torch.Tensor, filename_prefix: str = "output/16bit_",
             format: str = "PNG", tonemap: str = "Clip (Standard)") -> Dict:
...
def write_exr_openexr(filepath: str, channels: Dict[str, np.ndarray], compression: str = "ZIP", 
                      pixel_type: str = "HALF", metadata: Optional[Dict[str, Any]] = None) -> None:
    import OpenEXR
    import Imath
    ...
    header['compression'] = comp_map.get(compression, Imath.Compression(Imath.Compression.ZIP_COMPRESSION))
    
    ptype = Imath.PixelType(Imath.PixelType.HALF) if pixel_type == "HALF" else Imath.PixelType(Imath.PixelType.FLOAT)
...
class SimpleEXRWriter:
    ...
    def write(self, filepath: str, channels: Dict[str, np.ndarray], compression: str = "ZIP", 
              pixel_type: str = "HALF", metadata: Optional[Dict[str, Any]] = None) -> None:
        if not channels:
            raise ValueError("No channels provided")
        ...
        with open(filepath, 'wb') as f:
            ...
            self._write_header(f, width, height, channel_list, compression, pixel_type, metadata)
            self._write_scanlines(f, channels, channel_list, width, height, compression, pixel_type)
    
    def _write_header(self, f, width: int, height: int, channel_list: List[str], compression: str,
                      pixel_type: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        self._write_attribute(f, "channels", "chlist", self._encode_channel_list(channel_list, pixel_type))
        comp_value = EXR_COMPRESSION.get(compression, 3)
        if comp_value == 3: comp_value = 2 # ZIPS fallback
        self._write_attribute(f, "compression", "compression", struct.pack('<B', comp_value))
        self._write_attribute(f, "dataWindow", "box2i", struct.pack('<iiii', 0, 0, width - 1, height - 1))
        self._write_attribute(f, "displayWindow", "box2i", struct.pack('<iiii', 0, 0, width - 1, height - 1))
        self._write_attribute(f, "lineOrder", "lineOrder", struct.pack('<B', EXR_LINE_ORDER["INCREASING_Y"]))
        self._write_attribute(f, "pixelAspectRatio", "float", struct.pack('<f', 1.0))
        self._write_attribute(f, "screenWindowCenter", "v2f", struct.pack('<ff', 0.0, 0.0))
        ...
        reserved_attrs = {
            ...
            "screenWindowWidth", "tiles", "version", "chunkCount"
        }
    ...
    def _write_scanlines(self, f, channels: Dict[str, np.ndarray], channel_list: List[str], 
                         width: int, height: int, compression: str, pixel_type: str) -> None:
        ...
        for y in range(height):
            ...
            for channel_name in channel_list:
                ...
                row = channel[y, :]
                row_bytes = float16_to_bytes(row) if pixel_type == "HALF" else float32_to_bytes(row)
        ...
        for offset in offsets: f.write(struct.pack('<Q', offset))
        f.seek(current_pos)
...
class RadianceSaveEXR:
    ...
    def save_exr(
        ...
        for i in range(batch_size):
            try:
                ...
                img_out = self._convert_color_space(img, input_color_space, output_color_space)
                ...
                channels = self._build_channels(img_out, alpha, channel_format, alpha_mode)
                ...
                success = self._write_exr_with_fallback(
                    filepath, channels, img_out, alpha, alpha_mode,
                    compression, pixel_type, bit_depth, frame_metadata,
                    use_openexr, writer
                )
    ...
    def _convert_color_space(self, img: np.ndarray, input_cs: str, output_cs: str) -> np.ndarray:
        img_rgb = img[..., :3].copy()
        if input_cs == "Raw" or output_cs == "Same as Input": return img_rgb
        
        if input_cs == "sRGB (Gamma)":
            img_linear = srgb_to_linear(img_rgb)
            img_aces = apply_matrix_transform(img_linear, SRGB_TO_ACESCG) if SRGB_TO_ACESCG is not None else img_linear
        elif input_cs == "ACEScg":
            img_aces = img_rgb
        else:
            img_aces = apply_matrix_transform(img_rgb, SRGB_TO_ACESCG) if SRGB_TO_ACESCG is not None else img_rgb
        
        if output_cs == "Linear (ACEScg)": return img_aces
        elif output_cs == "Linear (sRGB)": return apply_matrix_transform(img_aces, ACESCG_TO_SRGB)
    ...
    def _get_alpha(self, img: np.ndarray, alpha_mode: str) -> Optional[np.ndarray]:
        if alpha_mode == "From Image" and img.shape[-1] == 4: return img[..., 3]
        elif alpha_mode == "Solid White": return np.ones(img.shape[:2], dtype=np.float32)
        elif alpha_mode == "Solid Black": return np.zeros(img.shape[:2], dtype=np.float32)
        return None
    
    def _build_channels(self, img_rgb: np.ndarray, alpha: Optional[np.ndarray], channel_format: str, alpha_mode: str) -> Dict[str, np.ndarray]:
        channels = {}
        if channel_format in ["RGB", "RGBA", "ACEScg"]:
            channels["R"], channels["G"], channels["B"] = img_rgb[..., 0], img_rgb[..., 1], img_rgb[..., 2]
        elif channel_format in ["BGR", "BGRA"]:
            channels["B"], channels["G"], channels["R"] = img_rgb[..., 0], img_rgb[..., 1], img_rgb[..., 2]
        
        if alpha_mode != "None" and alpha is not None: channels["A"] = alpha
        return channels
    
    def _write_exr_with_fallback(self, filepath, channels, img_rgb, alpha, alpha_mode, compression, pixel_type, bit_depth, metadata, use_openexr, writer) -> bool:
        if use_openexr:
            try:
        ...
        if write_exr_cv2(filepath, cv_img, bit_depth, compression): return True
