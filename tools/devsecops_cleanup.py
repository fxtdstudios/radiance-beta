...
def scan_and_clean(root: Path, dry_run: bool = True):
    ...
    for root_dir, dirs, files in os.walk(root):
        ...
        for file in files:
            ...
            if is_sensitive(file_path):
                logger.warning(f"SENSITIVE FILE DETECTED: {file_path} (Quarantined/Skipped)")
                sensitive_count += 1
                continue
            ...
            if should_delete(file_path):
                ...
                if dry_run:
                    logger.info(f"[DRY-RUN] Would delete file: {file_path} ({size} bytes)")
                else:
                    try:
                        file_path.unlink()
        ...
        for d in to_remove:
            ...
            if dry_run:
                logger.info(f"[DRY-RUN] Would remove directory: {dir_to_delete} (~{dir_size} bytes)")
            else:
                try:
    ...
    if sensitive_count > 0:
        logger.warning(f"SENSITIVE FILES FOUND: {sensitive_count} (Please review manually)")
