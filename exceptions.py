...
import logging
import functools
from typing import Any, Callable, TypeVar, Optional, Type, Tuple, Union
...
def validate_image_input(
    ...
    if ndim < min_dims or ndim > max_dims:
        raise RadianceImageError(
            f"Expected {min_dims}-{max_dims}D tensor, got {ndim}D",
            node_name=node_name
        )
    
    if require_batch and ndim == 3:
        raise RadianceImageError(
            "Missing batch dimension. Expected shape (B, H, W, C)",
            node_name=node_name
        )
    ...
    if any(s == 0 for s in image.shape):
        raise RadianceImageError(
            f"Image has zero dimension: {image.shape}",
            node_name=node_name
        )
